...
function makeCharacter() {
  ...
  const shadow = new Graphics()
    .ellipse(0, 2, 33, 8)
    .fill({ color: 0x172019, alpha: 0.17 })
  ...
  const backUpper = new Graphics().roundRect(-4, 0, 8, 29, 4).fill(0x334a42)
  const backForearm = new Graphics().roundRect(-3.5, 24, 7, 27, 3.5).fill(0xc49b78)
  backArm.addChild(backUpper, backForearm)
  backArm.pivot.set(0, 2)
  body.addChild(backArm)

  const backLeg = new Container()
  backLeg.position.set(-7, 76)
  const backThigh = new Graphics().roundRect(-5, 0, 10, 34, 5).fill(0x303433)
  const backCalf = new Graphics().roundRect(-4.5, 28, 9, 35, 4).fill(0x414643)
  const backShoe = new Graphics().roundRect(-5, 58, 22, 7, 3).fill(0x1c201e)
  backLeg.addChild(backThigh, backCalf, backShoe)
  ...
  const shirtDetail = new Graphics().roundRect(-2, 36, 4, 37, 2).fill({ color: 0xece7dc, alpha: 0.48 })
  body.addChild(shirtDetail)

  const neck = new Graphics().roundRect(-5, 22, 10, 15, 4).fill(0xc49b78)
  body.addChild(neck)

  const head = new Graphics().ellipse(0, 12, 13, 16).fill(0xcda582)
  body.addChild(head)
  ...
  const ear = new Graphics().ellipse(13, 13, 2.5, 4).fill(0xbc8e6c)
  body.addChild(ear)

  const eye = new Graphics().circle(6.5, 10, 1.25).fill(0x222522)
  body.addChild(eye)

  const nose = new Graphics().moveTo(11, 12).lineTo(14, 14).lineTo(10.5, 15).stroke({ color: 0x9f7458, width: 1 })
  body.addChild(nose)

  const frontLeg = new Container()
  frontLeg.position.set(7, 76)
  const frontThigh = new Graphics().roundRect(-5, 0, 10, 35, 5).fill(0x3a403d)
  const frontCalf = new Graphics().roundRect(-4.5, 29, 9, 35, 4).fill(0x4a4e4b)
  const frontShoe = new Graphics().roundRect(-5, 59, 23, 7, 3).fill(0x1d211f)
  frontLeg.addChild(frontThigh, frontCalf, frontShoe)
  frontLeg.pivot.set(0, 2)
  body.addChild(frontLeg)

  const frontArm = new Container()
  frontArm.position.set(10, 43)
  const frontUpper = new Graphics().roundRect(-4, 0, 8, 29, 4).fill(0x4d675e)
  const frontForearm = new Graphics().roundRect(-3.5, 24, 7, 27, 3.5).fill(0xc9a07d)
  const hand = new Graphics().circle(0, 52, 4.2).fill(0xc9a07d)
  frontArm.addChild(frontUpper, frontForearm, hand)
...
function drawOutdoorScene(width, height) {
  ...
  const sky = new Graphics().rect(0, 0, width, height).fill(0xdfe6df)
  root.addChild(sky)

  const haze = new Graphics()
    .circle(width * 0.74, 118, 98)
    .fill({ color: 0xfff4d8, alpha: 0.78 })
  root.addChild(haze)

  const far = new Graphics()
  far.moveTo(0, groundY - 145)
  for (let x = 0; x <= width; x += 170) {
    const y = groundY - 150 - Math.sin(x * 0.006) * 42 - Math.cos(x * 0.0027) * 28
    far.lineTo(x, y)
  }
  ...
  mid.moveTo(0, groundY - 60)
  for (let x = 0; x <= width; x += 120) {
    const y = groundY - 64 - Math.sin(x * 0.011) * 24
    mid.lineTo(x, y)
  }
  ...
  root.addChild(pathEdge)

  for (let i = 0; i < 24; i += 1) {
    const x = 130 + i * 128
    const trunk = new Graphics().roundRect(x, groundY - 112 - (i % 3) * 15, 7, 112 + (i % 3) * 15, 4).fill({ color: 0x5e655d, alpha: 0.52 })
    const crown = new Graphics().ellipse(x + 4, groundY - 132 - (i % 3) * 14, 34 + (i % 4) * 4, 30 + (i % 2) * 6).fill({ color: i % 2 ? 0x718273 : 0x809080, alpha: 0.67 })
...
function drawBuilding(groundY) {
  ...
  const shadow = new Graphics().roundRect(-28, 340, 600, 38, 18).fill({ color: 0x1f2621, alpha: 0.12 })
  building.addChild(shadow)

  const shell = new Graphics()
    .roundRect(0, 0, 540, 352, 18)
    .fill({ color: 0x7a817a, alpha: 0.88 })
    .stroke({ color: 0x3f4540, width: 1.5, alpha: 0.72 })
  building.addChild(shell)

  const glass = new Graphics()
    .roundRect(24, 24, 492, 176, 12)
    .fill({ color: 0xdce6e2, alpha: 0.42 })
    .stroke({ color: 0xf7fbf8, width: 1.2, alpha: 0.64 })
  building.addChild(glass)

  for (let x = 55; x < 500; x += 82) {
    building.addChild(new Graphics().rect(x, 24, 1.5, 176).fill({ color: 0x53605a, alpha: 0.25 }))
  }
  ...
  const doorFrame = new Graphics()
    .roundRect(386, 250, 96, 102, 6)
    .fill(0x424844)
    .stroke({ color: 0xe4e9e5, width: 1, alpha: 0.3 })
  building.addChild(doorFrame)

  const doorPanel = new Graphics()
    .roundRect(392, 256, 82, 96, 4)
    .fill({ color: 0xcddbd5, alpha: 0.78 })
    .stroke({ color: 0xf8fbfa, width: 1.5, alpha: 0.62 })
  doorPanel.pivot.set(392, 0)
  doorPanel.position.x = 392
  building.addChild(doorPanel)

  const handle = new Graphics().circle(462, 306, 3.2).fill(0x2b302d)
  building.addChild(handle)

  const threshold = new Graphics().rect(378, 350, 113, 4).fill({ color: 0xefe8d7, alpha: 0.7 })
  building.addChild(threshold)
...
function drawInterior(width, height) {
  ...
  const ceilingGlow = new Graphics().rect(0, 0, width, 190).fill({ color: 0x9eb8aa, alpha: 0.08 })
  root.addChild(ceilingGlow)

  const floor = new Graphics().rect(0, groundY, width, height - groundY).fill(0x2a2e2a)
  root.addChild(floor)

  const backWall = new Graphics()
    .roundRect(160, 120, 1280, groundY - 120, 20)
    .fill({ color: 0x303a35, alpha: 0.92 })
    .stroke({ color: 0xe5eee8, width: 1, alpha: 0.13 })
  root.addChild(backWall)

  for (let x = 205; x < 1360; x += 190) {
    const panel = new Graphics().roundRect(x, 168, 150, 168, 12).fill({ color: 0xa5b8ae, alpha: 0.07 }).stroke({ color: 0xdfebe4, width: 1, alpha: 0.12 })
    root.addChild(panel)
  }
  ...
  const consoleBase = new Graphics().roundRect(1010, groundY - 142, 196, 142, 16).fill({ color: 0x101512, alpha: 0.9 }).stroke({ color: 0xdce8e0, width: 1.2, alpha: 0.24 })
  root.addChild(consoleBase)
  ...
  const consoleLine1 = makeText('PROPOSAL', 8, 0x9eb8aa, '600')
  consoleLine1.position.set(1043, groundY - 112)
  root.addChild(consoleLine1)
  const consoleLine2 = makeText('Pay supplier', 15, 0xe8f0eb, '500')
  consoleLine2.position.set(1043, groundY - 88)
  root.addChild(consoleLine2)
  const consoleLine3 = makeText('AUTHORITY = 0', 9, 0xd7a977, '600')
  consoleLine3.position.set(1043, groundY - 57)
  root.addChild(consoleLine3)

  const exitFrame = new Graphics().roundRect(196, groundY - 112, 92, 112, 6).fill(0x1e2521).stroke({ color: 0xdfe8e2, width: 1, alpha: 0.2 })
  root.addChild(exitFrame)
  const exitDoor = new Graphics().roundRect(202, groundY - 106, 80, 106, 4).fill({ color: 0x8aa096, alpha: 0.3 })
  root.addChild(exitDoor)
...
export class WorldRuntime {
  ...
  async init() {
    ...
    this.sceneRoot.addChild(this.outdoor, this.interior)
  ...
  tick = (ticker) => {
    if (!this.character || this.mode === 'transition' || this.inspectionOpen) {
      this.animateCharacter(ticker.deltaMS, false)
      return
    }
    ...
    const maxSpeed = running ? 7.1 : 4.2
  ...
  moveInside(deltaMS) {
    const step = this.velocity * deltaMS / 16.67
    this.character.x = clamp(this.character.x + step, 205, 1370)
    const viewport = this.host.clientWidth
  ...
  enterBuilding() {
    ...
    tl.to(this.character, { x: targetX, duration: 0.65 })
      ...
      .call(() => {
        ...
        this.interior.alpha = 0
        this.character.x = 430
        this.character.y = this.interior.groundY
  ...
  exitBuilding() {
    ...
    const tl = gsap.timeline({ defaults: { ease: 'power2.inOut' } })
    tl.to(this.character, { x: this.interior.exitX, duration: 0.55 })
      .to(this.character, { alpha: 0, duration: 0.38 })
      .to(this.interior, { alpha: 0, duration: 0.58 }, '-=0.2')
      .call(() => {
        this.interior.visible = false
        this.outdoor.visible = true
        this.outdoor.alpha = 1
        this.character.x = this.building.doorWorldX - 92
        this.character.y = this.outdoor.groundY
        this.character.alpha = 0
        this.cameraX = clamp(this.character.x - this.host.clientWidth * 0.36, 0, this.worldWidth - this.host.clientWidth)
        this.outdoor.x = -this.cameraX
      })
